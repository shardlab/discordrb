...
module Discordrb::API::Channel
  ...
  # @deprecated Please use {create_channel_invite} instead.
  # https://discord.com/developers/docs/resources/channel#create-channel-invite
  def create_invite(token, channel_id, max_age = 0, max_uses = 0, temporary = false, unique = false, reason = nil)
    create_channel_invite(token, channel_id, max_age: max_age, max_uses: max_uses, temporary: temporary, unique: unique, reason: reason)
  end

  # Create an invite for the channel.
  # https://discord.com/developers/docs/resources/channel#create-channel-invite
  def create_channel_invite(token, channel_id, max_age: :undef, max_uses: :undef, temporary: :undef, unique: :undef, target_type: :undef, target_user_id: :undef, target_application_id: :undef, target_users_file: :undef, role_ids: :undef, flags: :undef, reason: nil)
    body = { max_age:, max_uses:, temporary:, unique:, target_type:, target_user_id:, target_application_id:, role_ids:, flags: }.reject { |_, value| value == :undef }
