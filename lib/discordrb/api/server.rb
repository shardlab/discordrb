...
module Discordrb::API::Server
  ...
  def create_automod_rule(token, server_id, name:, event_type:, trigger_type:, actions:, trigger_metadata: :undef, enabled: :undef, exempt_roles: :undef, exempt_channels: :undef, reason: nil)
    Discordrb::API.request(
      :guilds_sid_auto_moderation_rules,
      server_id,
      :post,
      "#{Discordrb::API.api_base}/guilds/#{server_id}/auto-moderation/rules",
      { name:, event_type:, trigger_type:, trigger_metadata:, actions:, enabled:, exempt_roles:, exempt_channels: }.reject { |_, value| value == :undef }.to_json,
      content_type: :json,
      Authorization: token,
      'X-Audit-Log-Reason': reason
    )
