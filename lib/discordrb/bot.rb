...
module Discordrb
  ...
  class Bot
    ...
    def register_application_command(name, description, server_id: nil, default_permission: nil, type: :chat_input)
      ...
      description_localization_builder = Interactions::DescriptionLocalizationBuilder.new

      yield(builder, permission_builder, name_localization_builder, description_localization_builder) if block_given?

      resp = if server_id
               API::Application.create_guild_command(@token, profile.id, server_id, name, description, builder.to_a, name_localization_builder.to_a, description_localization_builder.to_a, default_permission, type)
             else
               API::Application.create_global_command(@token, profile.id, name, description, builder.to_a, name_localization_builder.to_a, description_localization_builder.to_a, default_permission, type)
             end
