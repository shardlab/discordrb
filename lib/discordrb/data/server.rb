  ...
  class Server
    ...
    def search_messages(
      ...
      options = {
        ...
        attachment_filename: Array(file_names).map { |file| file.is_a?(Attachment) ? file.filename : file },
      ...
      # Due to speed optimizations, search may return slightly less messages
      # than the limit specified when messages have not been accessed in a very
      # long time. This means that we should not rely on the length of the messages array
      # to paginate results. This is usually going to result in one extra API call when we
      # are paginating through results because we can't do `page.count < 25`, but that's okay.
