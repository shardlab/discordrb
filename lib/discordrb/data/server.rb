...
module Discordrb
  ...
  class Server
    ...
    # @param channels [Array<Channel, Integer, String>, Channel, Integer, String, nil] Get messages sent in these channels.
    ...
    # @param reply_users [Array<#resolve_id>, #resolve_id, nil] Get messages that replied to these users.
    ...
    # @note Messages with GIFs sent before February 24, 2026 may not be returned under the `gif` embed type when using the `embed_types:` parameter.
    ...
    def search_messages(
      ...
      mentions: nil, mentions_everyone: nil, pinned: nil, has: nil, embed_types: nil, embed_providers: nil, link_hosts: nil,
      file_names: nil, file_extensions: nil, sort_by: nil, sort_order: :descending, include_nsfw: true, role_mentions: nil,
      reply_users: nil, reply_messages: nil
    )
      ...
      raise ArgumentError, "The 'role_mentions' parameter cannot include the `@everyone` role" if options[:mentions_role_id]&.any?(@id)
      ...
      # points should be when there are no more results, or when the limit has been exceeded.
