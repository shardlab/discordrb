...
module Discordrb
  ...
  class SoundboardSound
    ...
    # Check if the soundboard sound is a custom sound.
    # @return [true, false] If the soundboard sound is a custom sound.
    def custom?
      !@server.nil?
    end
    ...
    def creator
      ...
      me = @server.bot
      return unless me.can_create_server_expressions? || me.can_manage_emojis?
    ...
    # @!visibility private
    def inspect
      "<SoundboardSound id=#{@id} name=\"#{@name}\" volume=#{@volume} available=#{@available}>"
    end
