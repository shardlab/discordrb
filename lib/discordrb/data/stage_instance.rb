...
module Discordrb
  ...
  class StageInstance
    ...
    # @param privacy_level [Integer] The new privacy level (`1` or `2`) of the stage instance.
    # @param reason [String, nil] The reason to show in the audit log for updating the stage instance.
    # @return [nil]
    def modify(topic: :undef, privacy_level: :undef, reason: nil)
      data = { topic: topic, privacy_level: privacy_level, reason: reason }
      update_data(JSON.parse(API::Channel.update_stage_instance(@bot.token, @channel.id, **data)))
      nil
    end
