...
module Discordrb
  ...
  class Channel
    ...
    # @param auto_archive_duration [60, 1440, 4320, 10080] How long before a thread is automatically archived.
    ...
    def start_thread(name, auto_archive_duration, message: nil, type: 11)
      ...
      data = if message
               @bot.client.start_thread_with_message(@id, message_id, name: name, auto_archive_duration: auto_archive_duration)
             else
               @bot.client.start_thread_without_message(@id, name: name, auto_archive_duration: auto_archive_duration, type: type)
             end
      Channel.new(data, @bot, @server)
    end
