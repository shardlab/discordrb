...
module Discordrb
  ...
  class AutoModRule
    ...
    def modify(
      ...
      update_data(JSON.parse(API::Server.update_automod_rule(@bot.token, @server.id, @id, **data, reason: reason)))
      nil
    end

    # Delete the auto moderation rule. Use this with caution, as it cannot be undone.
    # @param reason [String, nil] The reason to show in the server's audit log for deleting the auto moderation rule.
    # @return [nil]
    ...
    def exempt?(other)
      ...
      when Discordrb::Member
        other.bot_account? || other.can_manage_server? || other.roles.any? { |key| @exempt_roles.any?(key.id) }
      when Discordrb::Channel
        @exempt_channels.any?(other.id)
      when Discordrb::Role
        @exempt_roles.any?(other.id)
      else
    ...
    def update_data(new_data)
      ...
      @trigger = Trigger.new(new_data['trigger_metadata'], @bot)
    ...
    class Trigger
      ...
      # @!visibility private
      def inspect
        "<Trigger type=#{@type} keyword_filter=#{@keyword_filter.inspect} regex_patterns=#{@regex_patterns.inspect}>"
      end
    end
    ...
    class Action
      ...
      # @!visibility private
      def inspect
        "<Action type=#{@type} custom_message=#{@custom_message.inspect} timeout_duration=#{@timeout_duration.inspect}>"
      end
