...
describe Discordrb::Message do
  ...
  describe '#reply!' do
    ...
    it 'responds with a message_reference' do
      expect(message).to receive(:respond).with(content, false, nil, nil, hash_including(:replied_user), message, nil, nil)

      message.reply!(content)
    end

    it 'sets replied_user in allowed_mentions' do
      expect(message).to receive(:respond).with(content, false, nil, nil, { replied_user: mention }, message, nil, nil)
    ...
    context 'when allowed_mentions is false' do
      ...
      it 'sets parse to an empty array add merges the mention_user param' do
        expect(message).to receive(:respond).with(content, false, nil, nil, { parse: [], replied_user: mention }, message, nil, nil)
    ...
    context 'when allowed_mentions is an AllowedMentions' do
      ...
      it 'converts it to a hash to set the replied_user key' do
        expect(message).to receive(:respond).with(content, false, nil, nil, hash, message, nil, nil)
        message.reply!(content, allowed_mentions: allowed_mentions, mention_user: mention_user, poll: nil)
      end
    end
  end
  ...
  describe '#respond' do
    ...
    let(:poll) { instance_double('Discordrb::Poll::Builder') }

    it 'forwards arguments to Channel#send_message' do
      expect(channel).to receive(:send_message).with(content, tts, embed, attachments, allowed_mentions, message_reference, components, poll)

      message.respond(content, tts, embed, attachments, allowed_mentions, message_reference, components, poll)
    end
  end
end
